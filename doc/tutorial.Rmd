...
beeswarm(word.count~FileOrdered, 
         ...
         main="Inaugural Speeches of Republican")
...
presid.summary=tbl_df(sentence.list.sel)%>%
...
```{r}
sentence.list.inaug=sentence.list%>%filter(type=="inaug", File%in%sel.comparison, Term==1)
...
topic.summary=tbl_df(corpus.list.df)%>%
              filter(type%in%c("inaug"), File%in%sel.comparison)%>%
